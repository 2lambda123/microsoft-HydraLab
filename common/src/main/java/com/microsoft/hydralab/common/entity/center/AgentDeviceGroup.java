...
public class AgentDeviceGroup {
  @Transient private static final int DEFAULT_ANALYSIS_COUNT = 3;
  ...
  private ConcurrentMap<String, Integer> availableAnalysisTaskCount =
  ...
  public void initWithAgentUser(AgentUser agentUser) {
    ...
    for (AgentFunctionAvailability functionAvailability :
         functionAvailabilities) {
      if (AgentFunctionAvailability.AgentFunctionType.ANALYSIS_RUNNER.equals(
              functionAvailability.getFunctionType()) &&
          functionAvailability.isEnabled()) {
        availableAnalysisTaskCount.put(functionAvailability.getFunctionName(),
                                       3);
      }
    }
  }
  ...
  public void runAnalysisTask(String runnerType) {
    availableAnalysisTaskCount.put(runnerType,
                                   availableAnalysisTaskCount.getOrDefault(
                                       runnerType, DEFAULT_ANALYSIS_COUNT) -
                                       1);
  }
  ...
  public void finishAnalysisTask(String runnerType) {
    ...
    availableAnalysisTaskCount.put(
        runnerType, availableAnalysisTaskCount.get(runnerType) + 1);
  }
}
